...
// Pause ranges (ms) — tuned for a calm idle cadence that still feels alive.
const PAUSE_SHORT_MS = [1000, 3000];
const PAUSE_LONG_MS = [5000, 12000];
const PAUSE_SIBLING_BLOCK_MS = [5000, 10000];
const PAUSE_POST_CLOSE_MS = [2000, 5000];
const THINKING_PHRASE_ROTATE_MS = [3000, 5000];
const COMPLETION_BUBBLE_DURATION_MS = 3000;

let lastSoundIndex = -1;
let soundsEnabled = true;

function setSoundsEnabled(enabled) { soundsEnabled = enabled; }
function getSoundsEnabled() { return soundsEnabled; }
...
class WalkerCharacter {
  constructor(config, manager) {
    ...
    this.pauseEndTime = Date.now() + randomInRange(...PAUSE_SHORT_MS);
  ...
  enterPause() {
    this.isWalking = false;
    this.isPaused = true;
    this.pauseEndTime = Date.now() + randomInRange(...PAUSE_LONG_MS);
    if (this.window && !this.window.isDestroyed())
  ...
  _updateThinkingPhrase() {
    const now = Date.now();
    if (!this.currentPhrase || now - this.lastPhraseUpdate > randomInRange(...THINKING_PHRASE_ROTATE_MS)) {
  ...
  showCompletionBubble() {
    ...
    this.completionBubbleExpiry = Date.now() + COMPLETION_BUBBLE_DURATION_MS;
    this.lastPhraseUpdate = 0;
  ...
  playCompletionSound() {
    ...
    const target = this._getSoundWindow();
    if (!target || target.isDestroyed()) return;

    const dispatch = () => {
      if (target.isDestroyed()) return;
      target.webContents.send('sound:play', { url: fileUrl, volume: 0.6 });
    };
    if (this._soundWindowReady) dispatch();
    else target.webContents.once('did-finish-load', () => {
      this._soundWindowReady = true;
      dispatch();
    });
  }
...
click anywhere outside to dismiss, then click us again to start chatting.`;
  ...
  closePopover() {
    ...
    this.pauseEndTime = Date.now() + randomInRange(...PAUSE_POST_CLOSE_MS);
  }
